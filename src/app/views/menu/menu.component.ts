...
export class MenuComponent implements OnInit {
    ...
    constructor(public baseCtrl: BaseMethodsService, public userService: UserService, @Inject(PLATFORM_ID) private platformId: any) {
    ...
    ngOnInit(): void {
        this.userName = this.baseCtrl.getHandleStorageData(
            'userName'
        );
    }
    ...
    onChangeLanguage(language: string) {
        if (this.baseCtrl.isBrowser) {
            let url = '';
            if (window.location.pathname.toString().split('/')[1] == 'en' ||
